...
export class HtmlReporter {
  ...
  generateReport(oldFolder: string, newFolder: string, comparisonResults: ComparisonResult[]): string {
    ...
    <head>
      ...
      <script src="https://cdnjs.cloudflare.com/ajax/libs/jsondiffpatch/0.4.2/jsondiffpatch.umd.js"></script>
      <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/jsondiffpatch/0.4.2/formatters-styles/html.css" />
      <style>
        ...
        .jsondiffpatch-delta { font-family: monospace; font-size: 14px; }
        .jsondiffpatch-added { background: #e6ffed; }
        .jsondiffpatch-deleted { background: #ffecec; text-decoration: line-through; }
        .jsondiffpatch-modified { background: #fff5cc; }
      </style>
    </head>
    <body>
      <div class="container">
          ...
          <div class="section">
            ...
            ${!result.matched && result.differences ? `
              ...
              <details>
                ...
                <div id="diff-${index}"></div>
              </details>
              <script>
                const oldData${index} = ${JSON.stringify(result.oldContent)};
                const newData${index} = ${JSON.stringify(result.newContent)};
                const delta${index} = jsondiffpatch.diff(oldData${index}, newData${index});
                const diffHtml${index} = jsondiffpatch.formatters.html.format(delta${index}, oldData${index});
                document.addEventListener("DOMContentLoaded", () => {
                  document.getElementById("diff-${index}").innerHTML = diffHtml${index};
                });
              </script>
            ` : ''}
          </div>
